...
def run_report():
    ...
    # BUG 3: Silent logic bug
    # Wrong filter silently removes most valid transactions
    filtered = df[df["amount"] > 10000]

    merchant_summary = (
        filtered.groupby("merchant_id")
        .agg({"amount": "sum"})
        .reset_index()
    )
...
'''
    ...
    ai_calls = len(
        [e for e in result2["healing_log"] if not e.get("from_memory")]
    )

    print(
        f"Status: {result2['final_status']} | "
        f"Attempts: {result2['total_attempts']} | "
        f"LLM calls: {ai_calls}"
    )
    ...
    print("\n── agent_memory.db contents ─────────────────────────────")

    for fp, err, ok, ts in rows:
        icon = "✅" if ok else "❌"
        print(f"{icon} {ts[:16]} | {fp} | {err}...")
    ...
    print("\n[RUN 2] Running healer again with SAME pipeline...")
    ...
    ai_calls_2 = len(
        [e for e in result3["healing_log"] if not e.get("from_memory")]
    )

    cached_2 = len(
        [e for e in result3["healing_log"] if e.get("from_memory")]
    )

    print(
        f"\nRun 2: LLM calls={ai_calls_2} | "
        f"From cache={cached_2}"
    )

    try:
        q1 = input(
            "\n1. Which bug did the agent miss and why? "
        ).strip()

        q2 = input(
            "2. How would you catch that logic bug in production? "
        ).strip()
    ...
    print("\nReflection logged.")
    print("Key lesson: Tracebacks = fixable. Silent wrong data = dangerous.")
